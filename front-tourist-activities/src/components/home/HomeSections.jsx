...
export const PackCard = ({ i, pack, handleAbrirPack }) => {
  return (
    <div
      onClick={() => handleAbrirPack(i)}
      className={`Act-card ${pack.color}`}
    >
      <img
        className="Pack-icono"
        src={`/img/pack-recom/${pack.tipo}.svg`}
        alt="icono pack actividades"
      />
...
export const PackDetails = ({
  ...
  actividades,
  setPackAbierto,
  handleAgregarACalendario,
}) => {
  return (
    <div className="Pack-overlay" onClick={() => setPackAbierto(null)}>
      <div
        className={`Pack-detalles ${listaDePacks[packAbierto].color}`}
        onClick={(e) => e.stopPropagation()}
      >
        <span className="Pack-cerrar" onClick={() => setPackAbierto(null)}>
          X
        </span>
        ...
        {actividades
          ...
          .map((actividad) => (
            <div
              key={actividad._id}
              className="Pack-activity Pack-activity--abierto"
            >
              <div className="Activity Activity--abierto">
                <h3 className="Activity-h3 Activity-h3--abierto">
                  {actividad.titulo}
                </h3>
                <p className="Activity-p Activity-p--abierto">
                  {actividad.descripcion}
                </p>
              </div>
              <button
                className="Activity-link Activity-link--abierto Activity-link--pack"
                onClick={() => handleAgregarACalendario(actividad)}
              >
    ...
    <div className="Activities-act" key={actividad._id}>
      <div className="Activities-icondiv">
        <img
          ...
          src={`/img/act-recom/${actividad.tipo}.svg`}
          alt="Icono actividad"
        />
      </div>
  ...
  return (
    <div className="Actividades-filtros">
      {tipos.map((tipo) => (
        <button
          key={tipo.valor}
          className={`Actividad-filtro ${
            tipoSeleccionado === tipo.valor ? `activo` : tipo.valor
          }`}
